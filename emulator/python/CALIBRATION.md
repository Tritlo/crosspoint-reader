...
Generic image tiers, large-spine indexing, warm-open fallback, and sleep entry are copied into explicit `models` fields
with their source workloads so native profile consumption does not depend on raw workload key order. Older
schema-version-1 profiles without those fields retain the original lookup.
