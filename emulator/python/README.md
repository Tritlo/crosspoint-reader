...
holds. `wait_for_activity()` recognizes Boot, Home, File Browser, Settings, Sleep, EPUB Reader, Reader menu, and chapter
selection. Every wait has
independent simulated-time and wall-time limits.
...
`clear_epub_cache("/book.epub")` is an explicit fixture-control operation for reproducible cold tests. It is accepted only
while File Browser and the panel are idle, runs the production `Epub::clearCache()` path on a firmware scheduler task, and
reports whether a cache existed, its recursive file count, and its simulated duration. The X4 profile uses `6,471 us`
(the seven-sample p50) for a missing cache and `74,036 + 17,504 * fileCount us` for a populated cache. Eleven
populated observations fit within
`12.894%`; file bytes were not monotonic with duration, and all populated captures had three directories, so neither is
used as an unsupported predictor.

## Timing profiles

`Emulator.launch("x4")` automatically loads `emulator/profiles/x4-hardware-2026-07-12-v1.json`. The handshake and run
...
uv run crosspoint-calibrate analyze \
  ...
  --output ../profiles/x4-hardware-2026-07-12-v1.json
```
